...
$selectedEquipment = CIBlockElement::GetPropertyValues(EVENTS_IBLOCK_ID, [
    'ID' => $ID
], false, ['ID' => ADDITIONAL_EQUIPMENT_PROPERTY_ID])->Fetch()[ADDITIONAL_EQUIPMENT_PROPERTY_ID];

$standartEquipment = [];
$usingStandsIds = CIBlockElement::GetPropertyValues(EVENTS_IBLOCK_ID, [
    'ID' => $ID
], false, ['ID' => USING_STANDS_PROPERTY_ID])->Fetch()[USING_STANDS_PROPERTY_ID];
$obStandsOffers = CIBlockElement::GetList([],[
    'IBLOCK_ID' => STANDS_OFFERS_IBLOCK_ID,
    'PROPERTY_CML2_LINK' => $usingStandsIds
], false, false, [
    'ID',
    'NAME',
    'PROPERTY_EQUIPMENT'
]);
while($arStandsOffer = $obStandsOffers->Fetch()) {
...
var allStands = $allStands,
    ...
    $(function() {
        $("#tab_cont_seo_adv_seo_adv").remove();
        var vm = new Vue({
...
function getPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
{
    ...
    if ($settings["multiple"] == "Y") {
        ...
        $html .= '<select v-model="selectedEquipment" multiple name="' . $strHTMLControlName["VALUE"] . '[]"' . $size . $width . '>';
        ...
        foreach ($values as $property_value_id => $value) {
            ...
            $bWasSelect = false;
            $options = \CIBlockPropertyElementList::GetOptionsHtml($arProperty, [$value], $bWasSelect);
...
function showEquipmentField(
    ...
    if (($arUserType["PROPERTY_TYPE"] !== "F") || (!$bCopy)) {
        if ($bMultiple) {
            $html .= '<tr><td>';
            $html .= getPropertyFieldHtmlMulty(
                $property_fields,
                $values,
                [
...
$tabControl->BeginCustomField("ACTIVE_FROM", GetMessage("IBLOCK_FIELD_ACTIVE_PERIOD_FROM"),
...
$tabControl->BeginCustomField("ACTIVE_TO", GetMessage("IBLOCK_FIELD_ACTIVE_PERIOD_TO"),
...
if ($arTranslit["TRANSLITERATION"] == "Y") {
    ...
    $tabControl->BeginCustomField("CODE", GetMessage("IBLOCK_FIELD_CODE") . ":",
    ...
    <tr id="tr_CODE">
        ...
        <td style="white-space: nowrap;">
            ...
            <image id="code_link" title="<? echo GetMessage("IBEL_E_LINK_TIP") ?>" class="linked"
                   src="/bitrix/themes/.default/icons/iblock/<? if ($bLinked) {
                       echo 'link.gif';
                   } else {
                       echo 'unlink.gif';
    ...
    $tabControl->AddEditField("NAME", GetMessage("IBLOCK_FIELD_NAME") . ":", true, ["size" => 50, "maxlength" => 255],
        $str_NAME);
    $tabControl->AddEditField("CODE", GetMessage("IBLOCK_FIELD_CODE") . ":",
        $arIBlock["FIELDS"]["CODE"]["IS_REQUIRED"] === "Y", ["size" => 20, "maxlength" => 255], $str_CODE);
}
...
if (
    ...
    $tabControl->BeginCustomField("IBLOCK_ELEMENT_SECTION_ID", GetMessage("IBEL_E_MAIN_IBLOCK_SECTION_ID") . ":",
        false);
    ?>
            ...
            <div id="RESULT_IBLOCK_ELEMENT_SECTION_ID">
                <select name="IBLOCK_ELEMENT_SECTION_ID" id="IBLOCK_ELEMENT_SECTION_ID"
                    ...
                    <? foreach ($arDropdown as $key => $val): ?>
                        <option
                            value="<? echo $key ?>" <? if ($str_IBLOCK_SECTION_ID == $key) echo 'selected' ?>><? echo $val ?></option>
                    <? endforeach ?>
                </select>
            </div>
            <script type="text/javascript">
...
if (COption::GetOptionString("iblock", "show_xml_id", "N") == "Y") {
    $tabControl->AddEditField("XML_ID", GetMessage("IBLOCK_FIELD_XML_ID") . ":",
        $arIBlock["FIELDS"]["XML_ID"]["IS_REQUIRED"] === "Y", ["size" => 20, "maxlength" => 255, "id" => "XML_ID"],
        $str_XML_ID);
}

$tabControl->AddEditField("SORT", GetMessage("IBLOCK_FIELD_SORT") . ":",
    $arIBlock["FIELDS"]["SORT"]["IS_REQUIRED"] === "Y", ["size" => 7, "maxlength" => 10], $str_SORT);
?>
    <script>
...
if (!empty($PROP)):
    ...
    foreach ($PROP as $prop_code => $prop_fields):
        ...
        <tr id="tr_PROPERTY_<? echo $prop_fields["ID"]; ?>"<? if ($prop_fields["PROPERTY_TYPE"] == "F"): ?> class="adm-detail-file-row"<? endif ?>>
            ...
            <td width="60%"><? _ShowPropertyField('PROP[' . $prop_fields["ID"] . ']', $prop_fields,
                    $prop_fields["VALUE"], (($historyId <= 0) && (!$bVarsFromForm) && ($ID <= 0) && (!$bPropertyAjax)),
                    $bVarsFromForm || $bPropertyAjax, 50000, $tabControl->GetFormName(), $bCopy); ?></td>
    ...
    if ($arLinkedProp) {
        ...
        do {
            ...
            <tr id="tr_LINKED_PROP<? echo $arLinkedProp["ID"] ?>">
                <td colspan="2"><a
                        href="<? echo htmlspecialcharsbx(CIBlock::GetAdminElementListLink($arLinkedProp["IBLOCK_ID"], [
                            'set_filter'                              => 'Y',
                            'find_el_property_' . $arLinkedProp["ID"] => $ID,
                            'find_section_section'                    => -1
                        ])) ?>"><? echo CIBlock::GetArrayByID($arLinkedProp["IBLOCK_ID"],
                                "NAME") . ": " . $elements_name ?></a></td>
            </tr>
            <?
        ...
        <td>
            <table id="event-prices-table">
                ...
                <tr>
                    <td>Использовать цены другого языка</td>
                    <td>
                        <select name="useOnRU" v-model="useOnRU">
                            <option value=""></option>
                            <option value="EN">EN</option>
                        </select>
                    </td>
                    <td>
                        <select name="useOnEN" v-model="useOnEN">
                            <option value=""></option>
                            <option value="RU">RU</option>
                        </select>
                    </td>
...
<?
$tabControl->EndCustomField("EQUIPMENT_PRICES", '');

$tabControl->BeginCustomField("STANDART_PRICES", "Цены на стандартное оборудование");
?>
    <tr>
        <td>
            <table id="event-prices-table">
                ...
                <tbody>
                <tr v-for="(index, eqId) in standartEquipment">
                    <td>
                        <input type="hidden" name="steq[{{index}}][ID]" :value="eqId">
                        {{ allEquipment[eqId].NAME }}
                    </td>
                    <td>
                        <input v-model="PRICE_RU[eqId]" name="steq[{{index}}][PRICE_RU]" type="text" size="10"
                               :value="allEquipment[eqId].PRICE_RU">
                    </td>
                    <td>
                        <input v-model="PRICE_EN[eqId]" name="steq[{{index}}][PRICE_EN]" type="text" size="10"
...
<?
$tabControl->EndCustomField("STANDART_PRICES", '');
...
$tabControl->BeginCustomField("PREVIEW_TEXT", GetMessage("IBLOCK_FIELD_PREVIEW_TEXT"),
...
elseif (COption::GetOptionString("iblock", "use_htmledit", "Y") == "Y" && $bFileman): ?>
    ...
    <tr id="tr_PREVIEW_TEXT_TYPE">
        ...
        <td><input type="radio" name="PREVIEW_TEXT_TYPE" id="PREVIEW_TEXT_TYPE_text"
                ...
                for="PREVIEW_TEXT_TYPE_html"><? echo GetMessage("IBLOCK_DESC_TYPE_HTML") ?></label></td>
    </tr>
...
$tabControl->BeginNextFormTab();
$tabControl->BeginCustomField("DETAIL_PICTURE", GetMessage("IBLOCK_FIELD_DETAIL_PICTURE"),
...
$tabControl->BeginCustomField("DETAIL_TEXT", GetMessage("IBLOCK_FIELD_DETAIL_TEXT"),
...
<? else: ?>
    <tr id="tr_DETAIL_TEXT_TYPE">
        <td><? echo GetMessage("IBLOCK_DESC_TYPE") ?></td>
        <td><input type="radio" name="DETAIL_TEXT_TYPE" id="DETAIL_TEXT_TYPE_text"
                   value="text"<? if ($str_DETAIL_TEXT_TYPE != "html") echo " checked" ?>> <label
                for="DETAIL_TEXT_TYPE_text"><? echo GetMessage("IBLOCK_DESC_TYPE_TEXT") ?></label> / <input type="radio"
                                                                                                            name="DETAIL_TEXT_TYPE"
                                                                                                            id="DETAIL_TEXT_TYPE_html"
                                                                                                            value="html"<? if ($str_DETAIL_TEXT_TYPE == "html") echo " checked" ?>>
            <label for="DETAIL_TEXT_TYPE_html"><? echo GetMessage("IBLOCK_DESC_TYPE_HTML") ?></label></td>
    </tr>
...
?>
    <tr class="adm-detail-valign-top">
        ...
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_META_TITLE", $str_IPROPERTY_TEMPLATES,
                "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_META_TITLE",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_META_TITLE", $str_IPROPERTY_TEMPLATES, "E",
...
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_META_KEYWORDS",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_META_KEYWORDS",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_META_KEYWORDS", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_META_DESCRIPTION",
    GetMessage("IBEL_E_SEO_META_DESCRIPTION"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_META_DESCRIPTION",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_META_DESCRIPTION",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_META_DESCRIPTION", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
...
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_PAGE_TITLE", $str_IPROPERTY_TEMPLATES,
                "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_PAGE_TITLE",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_PAGE_TITLE", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->AddSection("IPROPERTY_TEMPLATES_ELEMENTS_PREVIEW_PICTURE",
    GetMessage("IBEL_E_SEO_FOR_ELEMENTS_PREVIEW_PICTURE"));
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_ALT",
    GetMessage("IBEL_E_SEO_FILE_ALT"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_ALT",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_ALT",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_ALT", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_TITLE",
    GetMessage("IBEL_E_SEO_FILE_TITLE"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_TITLE",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_TITLE",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_TITLE", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_NAME",
    GetMessage("IBEL_E_SEO_FILE_NAME"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_NAME",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_PREVIEW_PICTURE_FILE_NAME",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_PREVIEW_PICTURE_FILE_NAME", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->AddSection("IPROPERTY_TEMPLATES_ELEMENTS_DETAIL_PICTURE",
...
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_ALT",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_DETAIL_PICTURE_FILE_ALT",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_ALT", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_DETAIL_PICTURE_FILE_TITLE",
    GetMessage("IBEL_E_SEO_FILE_TITLE"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_TITLE",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_DETAIL_PICTURE_FILE_TITLE",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_TITLE", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->BeginCustomField("IPROPERTY_TEMPLATES_ELEMENT_DETAIL_PICTURE_FILE_NAME",
    GetMessage("IBEL_E_SEO_FILE_NAME"));
?>
    <tr class="adm-detail-valign-top">
        <td width="40%"><? echo $tabControl->GetCustomLabelHTML() ?></td>
        <td width="60%"><? echo IBlockInheritedPropertyInput($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_NAME",
                $str_IPROPERTY_TEMPLATES, "E", GetMessage("IBEL_E_SEO_OVERWRITE")) ?></td>
    </tr>
<?
$tabControl->EndCustomField("IPROPERTY_TEMPLATES_ELEMENT_DETAIL_PICTURE_FILE_NAME",
    IBlockInheritedPropertyHidden($IBLOCK_ID, "ELEMENT_DETAIL_PICTURE_FILE_NAME", $str_IPROPERTY_TEMPLATES, "E",
        GetMessage("IBEL_E_SEO_OVERWRITE"))
);
?>
<?
$tabControl->AddSection("SEO_ADDITIONAL", GetMessage("IBLOCK_EL_TAB_MO"));
$tabControl->BeginCustomField("TAGS", GetMessage("IBLOCK_FIELD_TAGS") . ":",
...
<? if ($arShowTabs['sections']):
    ...
    $tabControl->BeginCustomField("SECTIONS", GetMessage("IBLOCK_SECTION"),
        ...
        <? if ($arIBlock["SECTION_CHOOSER"] != "D" && $arIBlock["SECTION_CHOOSER"] != "P"): ?>
            ...
            <td width="60%">
                <select name="IBLOCK_SECTION[]" size="14" multiple onchange="onSectionChanged()">
                    <option value="0"<? if (is_array($str_IBLOCK_ELEMENT_SECTION) && in_array(0,
                            $str_IBLOCK_ELEMENT_SECTION)
                    ) echo " selected" ?>><? echo GetMessage("IBLOCK_UPPER_LEVEL") ?></option>
                    <?
                    while ($ar_l = $l->GetNext()):
                        ?>
                        <option value="<? echo $ar_l["ID"] ?>"<? if (is_array($str_IBLOCK_ELEMENT_SECTION) && in_array($ar_l["ID"],
                            $str_IBLOCK_ELEMENT_SECTION)
                    ) echo " selected" ?>><? echo str_repeat(" . ",
                        $ar_l["DEPTH_LEVEL"]) ?><? echo $ar_l["NAME"] ?></option><?
                    endwhile;
                    ?>
                </select>
            </td>
        ...
        elseif ($arIBlock["SECTION_CHOOSER"] == "D"):?>
                    ...
                    <tr>
                        <td>
                            <script type="text/javascript">
                                ...
                                <?
                                $rsItems = CIBlockSection::GetTreeList(["IBLOCK_ID" => $IBLOCK_ID],
                                    ["ID", "NAME", "DEPTH_LEVEL"]);
                                $depth = 0;
                                $max_depth = 0;
                                $arChain = [];
            ...
            <td width="60%">
                ...
                <input type="button" value="<? echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD") ?>..."
                       onClick="jsUtils.OpenWindow('/bitrix/admin/iblock_section_search.php?lang=<? echo LANGUAGE_ID ?>&amp;IBLOCK_ID=<? echo $IBLOCK_ID ?>&amp;n=input_IBLOCK_SECTION&amp;m=y', 600, 500);">
            </td>
        <? endif; ?>
    </tr>
...
if ($arShowTabs['product_set']) {
    ...
    <td colspan="2"><?
        ...
        $arSets = false;
        CCatalogAdminProductSetEdit::setProductFormParams(['TYPE' => CCatalogProductSet::TYPE_SET]);
        if (0 < $intProductID) {
...
if ($arShowTabs['product_group']) {
    ...
    <td colspan="2"><?
        ...
        $arSets = false;
        CCatalogAdminProductSetEdit::setProductFormParams(['TYPE' => CCatalogProductSet::TYPE_GROUP]);
        if (0 < $intProductID) {
...
if ($arShowTabs['bizproc']):
        ...
        <tr class="heading">
            <td colspan="2">
                ...
                <? if (strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0): ?>
                    (<a href="<? echo htmlspecialcharsbx("/bitrix/admin/" . CIBlock::GetAdminElementEditLink($IBLOCK_ID,
                            $ID, [
                                "WF"                   => $WF,
        ...
        <? if (strlen($arDocumentState["STATE_NAME"]) > 0): ?>
            <tr>
                ...
                <td width="60%"><? if (strlen($arDocumentState["ID"]) > 0): ?><a
                        href="/bitrix/admin/bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>"><?endif;
                ...
                <tr>
                    ...
                    <td width="60%">
                        ...
                        foreach ($arTasks as $arTask) {
                            ?><a
                            href="/bitrix/admin/bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("",
    ...
    if ($ID > 0):
        ...
        if ($bStartWorkflowPermission):
            ...
            <tr>
                <td colspan="2" align="center">
                    <a href="/bitrix/admin/<?= MODULE_ID ?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("",
...
if ($arShowTabs['edit_rights']):
    ...
    IBlockShowRights(
        ...
        $obRights->GetRights(["count_overwrited" => true, "parents" => $str_IBLOCK_ELEMENT_SECTION]),
...
if (!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1):
    ...
    <input <? if ($bDisabled) {
        echo "disabled";
    } ?> type="submit" class="adm-btn-save" name="save" id="save" value="<? echo GetMessage("IBLOCK_EL_SAVE") ?>">
    <? if (!$bAutocomplete) {
    ?><input <? if ($bDisabled) {
        echo "disabled";
    ...
    <input <? if ($bDisabled) {
        echo "disabled";
    } ?> type="submit" class="button" name="dontsave" id="dontsave" value="<? echo GetMessage("IBLOCK_EL_CANC") ?>">
    <? if (!$bAutocomplete) {
    ?><input <? if ($bDisabled) {
        echo "disabled";
...
if (
    ...
    GetMessage("IBEL_E_IBLOCK_MANAGE_HINT"),
        ' <a href="/bitrix/admin/iblock_edit.php?type=' . htmlspecialcharsbx($type) . '&amp;lang=' . LANGUAGE_ID . '&amp;ID=' . $IBLOCK_ID . '&amp;admin=Y&amp;return_url=' . urlencode("/bitrix/admin/" . CIBlock::GetAdminElementEditLink($IBLOCK_ID,
                $ID, [
                    "WF"                   => ($WF == "Y" ? "Y" : null),
                    "find_section_section" => intval($find_section_section),
