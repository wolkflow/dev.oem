<?php


/*
 * Константы по времени
 */
define('TIME_DAY', 86400);
define('TIME_YEAR', 31536000);


/*
 * Код в сессии.
 */
define('SESSION_CODE', 'OEMEVENTS');

/*
 * Языки.
 */
define('LANG_RU', 'ru');
define('LANG_EN', 'en');

define('LANG_RU_UP', 'RU');
define('LANG_EN_UP', 'EN');


/*
 * Шаблон по умолчанию.
 */
define('DEFAULT_TEMPLATE_PATH', '/local/templates/.default');

/*
 * Сайт по умолчанию.
 */
define('SITE_DEFAULT', 's1');


/*
 * Тип плательщика по умолчанию.
 */
define('PERSON_TYPE_DETAULT', 1);


/*
 * Группы.
 */
define('GROUP_MANAGERS_ID', 9);
define('GROUP_PARTNERS_ID', 10);

/*
 * Валюта.
 */
define('CURRENCY_DEFAULT', 'EUR'); 
 
 
/*
 * Доставка.
 */
define('DELIVERY_DETAULT', 1);

/*
 * Тип по умолчанию.
 */
define('TYPE_DEFAULT', 'STANDARD');
 

// Инфоблоки.
define('EVENTS_IBLOCK_ID', 1);
define('STANDS_IBLOCK_ID', 2);
define('STANDS_OFFERS_IBLOCK_ID', 6);
define('OPTIONS_IBLOCK_ID', 5);
define('EQUIPMENT_IBLOCK_ID', 5);
define('SELECTED_STANDS_PROPERTY_ID', 32);
define('ADDITIONAL_EQUIPMENT_PROPERTY_ID', 15);
define('ADDITIONAL_EQUIPMENT_SECTION_ID', 10);
define('ADDITIONAL_SERVICES_SECTION_ID', 11);
define('USING_STANDS_PROPERTY_ID', 32);

define('EQUIPMENT_PROPERTY_ID', 30);

define('COLORS_ENTITY_ID', 3);
define('EXTENTS_ENTITY_ID', 4);
define('EQUIPMENT_COLORS_ENTITY_ID', 5);


// Инфоблоки.
define('IBLOCK_EVENTS_ID',       1);
define('IBLOCK_STANDS_ID',       2);
define('IBLOCK_STAND_OFFERS_ID', 6);
define('IBLOCK_LOCATIONS_ID',    7);
define('IBLOCK_PRODUCTS_ID',     8);
define('IBLOCK_PLACES_ID',       9);

// HL-блоки.
define('HLBLOCK_ORDER_SKETCHES_ID',  6);
define('HLBLOCK_PRICES_PRODUCTS_ID', 9);
define('HLBLOCK_PRICES_STANDS_ID',   10);
define('HLBLOCK_PARAMS_PRODUCTS_ID', 11);
define('HLBLOCK_PARAMS_TEMP_RENDER_STORAGE_ID', 12);
define('HLBLOCK_MAP_OBJECTS_ID', 13);

// Разделы.
define('SECTION_LOGOTYPES_ID', 13);
define('SECTION_BANNERS_ID', 58);

// Свойства.
define('IBLOCK_PROPERTY_SELECTED_STANDS_ID',               32);
define('IBLOCK_PROPERTY_SELECTED_PRODUCTS_STANDARD_ID',   108);
define('IBLOCK_PROPERTY_SELECTED_PRODUCTS_INDIVIDUAL_ID', 125);

define('HLBLOCK_PROPERTY_MAP_OBJECTS_TYPE_ID', 133);


define('VAT_DEFAULT',   18);
define('UNVAT_DEFAULT', 0.152542372881356);


// IP боевого сервера.
define('PRODUCTION_IP', '89.253.254.58');


define('HLBLOCK_USERS_API_ID', 7);



/*
 * Ошибки инфоблоков.
 */
define('IBLOCK_ERROR_TYPE_CRITICAL', 1);
define('IBLOCK_ERROR_TYPE_FAIL',     2);



/*
 * Разделы продукции.
 */
define('SECTION_PRODUCTS_EQUIPMENTS_ID', 58);
define('SECTION_PRODUCTS_SERVICES_ID',   59);
define('SECTION_PRODUCTS_MARKETINGS_ID', 60);

/*
 * Свойства продукции.
 */
define('SECTION_PRODUCTS_PROPERTY_TEXT_ID',                   15);
define('SECTION_PRODUCTS_PROPERTY_LINK_ID',                   12);
define('SECTION_PRODUCTS_PROPERTY_COMMENT_ID',                11);
define('SECTION_PRODUCTS_PROPERTY_FILE_ID',                   10);
define('SECTION_PRODUCTS_PROPERTY_COLOR_ID',                  13);
define('SECTION_PRODUCTS_PROPERTY_FORM_HANGING_STRUCTURE_ID', 14);
define('SECTION_PRODUCTS_PROPERTY_MODEL_ID',                 118);


define('ELEMENT_EVENTS_PROPERTY_PRODUCTS_STANDARD_ID', 108);
define('ELEMENT_EVENTS_PROPERTY_PRODUCTS_INDIVIDUAL_ID', 125);

